(function() {
  ...
  RoomSlider.prototype.resetCss = (function() {
    ...
    createStyle.innerHTML +=
      ".buttonCss{font-size:14px;position:absolute;;z-index:2;background:rgba(51,51,51,0.4);color:#fff;padding:4px;border-style:none;outline:none;cursor:pointer;display:none;}";
    createStyle.innerHTML += ".room{position:absolute;}"; //初始化room的css
    createStyle.innerHTML +=
      ".slide{font-size:0;overflow:hidden;position:relative;}"; //初始化slide样式
    createStyle.innerHTML +=
      ".nav{padding:0 4px;list-style:none;position:absolute;font-size:0px;background:rgba(222,222,222,0.4);bottom:10px;right:10px;text-align:center;-moz-border-radius:10px;border-radius:10px;}"; //nav样式
    createStyle.innerHTML +=
      ".navButton{display:inline-block;margin:6px 4px;background:#fff;width:8px;height:8px;-moz-border-radius:8px;border-radius:8px;cursor:pointer;}"; //导航点样式
    ...
    this.time = setInterval(function() {
      ...
      if (_this.imgIndex < _this.img.length) {
        if (left > -_this.slideWidth * _this.imgIndex) {
          _this.room.style.left =
            parseInt(_this.room.style.left) + _this.speed + "px";
        } else {
        ...
        if (left < 0) {
          _this.room.style.left =
            parseInt(_this.room.style.left) + -_this.speed * 2 + "px";
        } else {
          clearInterval(_this.time);
    ...
    this.time = setInterval(function() {
      ...
      if (_this.imgIndex == 1) {
        if (left > -_this.slideWidth * (_this.img.length - 1)) {
          ...
          clearInterval(_this.time);
          _this.room.style.left =
            -_this.slideWidth * (_this.img.length - 1) + "px";
        ...
        if (
          left < -(_this.slideWidth * (_this.imgIndex - 1) - _this.slideWidth)
        ) {
          _this.room.style.left = left - _this.speed + "px";
        } else {
          clearInterval(_this.time);
          _this.room.style.left =
            -(_this.slideWidth * (_this.imgIndex - 1) - _this.slideWidth) +
            "px";
    ...
    this.time = setInterval(function() {
      ...
      if (liTarget.index > _this.imgIndex) {
        if (left > -_this.slideWidth * (liTarget.index - 1)) {
          _this.room.style.left =
            parseInt(_this.room.style.left) +
            _this.speed * (liTarget.index - _this.imgIndex) +
            "px";
        } else {
          clearInterval(_this.time);
          _this.room.style.left =
            -_this.slideWidth * (liTarget.index - 1) + "px";
        ...
        if (left < -_this.slideWidth * (liTarget.index - 1)) {
          _this.room.style.left =
            parseInt(_this.room.style.left) -
            _this.speed * (_this.imgIndex - liTarget.index) +
            "px";
        } else {
          clearInterval(_this.time);
          _this.room.style.left =
